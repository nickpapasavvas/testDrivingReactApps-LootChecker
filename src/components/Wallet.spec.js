...
describe('Wallet', () => {
    ...
    // these are connected props (mapStateToProps,mapDispatchToProps)
    ...
    describe('when the user types into the wallet input', () => {
        ...
        describe('when the user makes a deposit', () => {
            beforeEach(()=>{
                wallet.find('.btn-deposit').simulate('click')
            })
            it('should dispatch the `deposit()` it receives from props with the local balance', () => {
               expect(mockDeposit).toHaveBeenCalledWith(parseInt(userBalance,10)) 
            });
        });
        describe('when the user makes a withdrawal', () => {
            beforeEach(()=>{
